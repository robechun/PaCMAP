...
def sample_labeled_dist(X, y, n_label_neighbors):
    ...
    for i in range(n):
        ...
        for sample_idx, label in sampled.items():
            distances[i, jj] = calculate_dist(X[i], X[sample_idx], distance_index=0)
...
def sample_labeled_pair(X, y, n_label_neighbors):
    ...
    for i in range(n):
        ...
        for sample_idx, label in sampled.items():
            ...
            pair_labeled[i * n_label_neighbors + jj][1] = calculate_dist(X[i], X[sample_idx], distance_index=0)
            jj += 1
...
def sample_FP_pair(X, pair_neighbors, n_neighbors, n_FP):
    ...
    for i in numba.prange(n):
        for k in numba.prange(n_FP):
            neighbor_pairs_for_i = pair_neighbors[i * n_neighbors: i * n_neighbors + n_neighbors]
            already_sampled_points = np.array([pair[1] for pair in neighbor_pairs_for_i])

            FP_index = sample_FP(
                n_FP,
                n,
                already_sampled_points
            )
    ...
    for i in numba.prange(n):
        for k in numba.prange(n_FP):
            ...
            FP_index = sample_FP(
                n_FP,
                n,
                pair_neighbors[i * n_neighbors: i *
...
def update_embedding_adam(Y, grad, m, v, beta1, beta2, lr, itr):
    ...
    n, dim = Y.shape
    lr_t = lr * math.sqrt(1.0 - beta2 ** (itr + 1)) / \
        (1.0 - beta1 ** (itr + 1))
    for i in numba.prange(n):
...
# TODO @robert add the labeled ones in here too for the loss
@numba.njit(
...
def pacmap_grad(Y, pair_neighbors, pair_label_neighbors, pair_MN, pair_FP, w_neighbors, w_label, w_MN, w_FP):
    ...
    # TODO weight for label (maybe same as NN?)
    for ta in range(pair_label_neighbors.shape[0]):
        i = pair_label_neighbors[ta, 0]
        j = pair_label_neighbors[ta, 1]
        d_ij = 1.0
        for d in range(dim):
            y_ij[d] = Y[i, d] - Y[j, d]
            d_ij += y_ij[d] ** 2
        loss[0] += w_label * (d_ij / (10.0 + d_ij))
        w1 = w_label * (20.0 / (10.0 + d_ij) ** 2)
        for d in range(dim):
            grad[i, d] += w1 * y_ij[d]
            grad[j, d] -= w1 * y_ij[d]
...
# TODO @robert add the weight for labled data
def find_weight(w_MN_init, itr):
    ...
    if itr < 100:
        ...
        w_FP = 1.0
        w_label = 1.5
    return w_MN, w_neighbors, w_label, w_FP
...
def generate_pair(X, y, n_neighbors, n_MN, n_FP, distance="euclidean", verbose=True):
    ...
    if y is not None:
        sampled_labeled_dist = sample_labeled_dist(X, y, n_neighbors)
        sig_label = np.maximum(np.mean(sampled_labeled_dist[:, 3:6], axis=1), 1e-10)
        scaled_dist = scale_dist(sampled_labeled_dist, sig_label, nbrs)

        pair_label_neighbors = sample_labeled_pair(X, y, 10)
    else:
        pair_label_neighbors = None

    if _RANDOM_STATE is None:
        print_verbose('Random state is None', verbose)
        pair_MN = sample_MN_pair(X, n_MN, option)
        pair_FP = sample_FP_pair(X, pair_neighbors, n_neighbors, n_FP)
    else:
        print_verbose('Random state is NOT - None', verbose)
...
def pacmap(
    ...
    print_verbose("shape items below", verbose)
    print_verbose(
        (pair_neighbors.shape, pair_label_neighbors.shape, pair_MN.shape, pair_FP.shape), verbose)

    for itr in range(num_iters):
        w_MN, w_neighbors, w_label, w_FP = find_weight(w_MN_init, itr)

        grad = pacmap_grad(Y, pair_neighbors, pair_label_neighbors, pair_MN,
                           pair_FP, w_neighbors, w_label, w_MN, w_FP)
        C = grad[-1, 0]
    ...
    return Y, intermediate_states, pair_neighbors, pair_label_neighbors, pair_MN, pair_FP


# TODO do i need to update this as well to account for the weight + other
# from the label stuff i added?
def pacmap_fit(
    X,
    embedding,
    n_dims,
    ...
    if Yinit is None or Yinit == "pca":
        ...
        Y = np.concatenate(
            [
                embedding,
                0.0001 *
                np.random.normal(size=[X.shape[0], n_dims]).astype(np.float32),
            ]
        )
    else:  # user_supplied matrix
        Yinit = Yinit.astype(np.float32)
...
def save(instance, common_prefix: str):
    ...
    print(
        f"To load the instance again, please do `pacmap.load({common_prefix})`.")
...
class PaCMAP(BaseEstimator):
    ...
    pair_neighbors: numpy.ndarray, optional
        The dimensions are: (n * n_neighbors, 2)
    ...
    def fit(self, X, y, init=None, save_pairs=True):
    ...
    def sample_pairs(self, X, y, save_tree):
        """
        ...
        if self.pair_neighbors is None:
            self.pair_neighbors, self.pair_label_neighbors, self.pair_MN, self.pair_FP, self.tree = generate_pair(
                X, y, self.n_neighbors, self.n_MN, self.n_FP, self.distance, self.verbose
            )
            print_verbose("Pairs sampled successfully.", self.verbose)
            print(self.pair_label_neighbors)
